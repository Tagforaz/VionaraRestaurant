...
import { LayoutDashboard, Package, LogOut, Menu, ChevronLeft } from 'lucide-react';
...
export const CourierLayout: React.FC<CourierLayoutProps> = ({ children }) => {
  ...
  const navItems = [
    { path: '/courier', label: 'Dashboard', icon: LayoutDashboard },
    { path: '/courier/deliveries', label: 'Çatdırılmalar', icon: Package },
  ];
    ...
    <div className="flex min-h-screen bg-background">
          ...
          >
            ...
            {!sidebarCollapsed && (
              <div className="flex-1 overflow-hidden">
                ...
                <p className="truncate text-xs text-muted-foreground">Kuryer</p>
              </div>
            )}
          </Link>
          <Button
      ...
      {mobileSidebarOpen && (
        ...
        >
          ...
          >
            <div className="flex h-16 items-center justify-between border-b border-sidebar-border px-4">
              <Link to="/courier" className="font-display text-xl font-bold">
                <span className="text-amber-600">V</span>
                <span className="text-sidebar-foreground">ionara</span>
              </Link>
            </div>
            ...
            <div className="absolute bottom-0 w-full border-t border-sidebar-border p-4">
              {user && (
                <div className="mb-3 rounded-lg bg-sidebar-accent p-3">
                  ...
                  <p className="text-xs text-sidebar-foreground/70">Kuryer</p>
                </div>
              )}
              <Button
                variant="ghost"
      ...
      <div className={cn('flex-1 transition-all duration-300', sidebarCollapsed ? 'lg:ml-20' : 'lg:ml-64')}>
        ...
        <header className="sticky top-0 z-30 hidden h-16 items-center justify-between border-b border-border bg-background px-6 lg:flex">
          <div>
            <h2 className="text-lg font-semibold text-foreground">Kuryer Paneli</h2>
          </div>
        ...
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-border bg-background px-4 lg:hidden">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-muted-foreground text-sm hidden sm:inline">/ Kuryer</span>
          </div>
          <div className="flex items-center gap-2">
            <Link to="/profile">
